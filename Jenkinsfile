...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit abdulganiu123/reddit:latest "
                       sh "docker push abdulganiu123/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image abdulganiu123/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.223.152.26 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.224.31.10:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#ganiyu-devsecops-cicd-alerts', //update and provide your channel name
