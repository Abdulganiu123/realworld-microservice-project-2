...
pipeline {
    ...
    stages {
        ...
        stage('Checkout To Mcroservice Branch'){
            steps{
                git branch: 'app-payment-service', url: 'https://github.com/Abdulganiu123/realworld-microservice-project.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t abdulganiu123/paymentservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker abdulganiu123/paymentservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push abdulganiu123/paymentservice:latest "
                    }
                }
            }
        }
